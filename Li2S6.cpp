 ...
 * the first 8 atoms belongs to Li2S6
...
/************
 * Read the xyz in the POSCAR.Li2S6 and POSCAR.MoSe2
************/
void readxyz(int atom)
{
    ...
    for(k = 1; k <= atom; k++)
    {
        ...
        tmp2 = atoms[k].y;
        tmp3 = atoms[k].z;
        atoms[k].x = tmp1 * 16.774658 + tmp2 * 0.0150410 - tmp3 * 0.0284433;
        atoms[k].y = tmp1 * 0.0161178652 + tmp2 * 16.124509188 - tmp3 * 0.00166694;
        atoms[k].z = tmp3 * 14.449205 - tmp1 * 0.01548765 - tmp2 * 0.010553012;
    }
    fclose(init);
...
// Calculate the center coordination of Li2S6
...
int main()
{
    int atom = 8;   // No. of atoms of Li2S6
    ...
    readxyz(atom);  // read Li2S6 and MoSe2
    center(atom);   // calculate the center of Li2S6

    rotation(atom, alpha, beta, gamma);  // rotate the Li2S6
