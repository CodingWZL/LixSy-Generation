 ...
 * the first 6 atoms belongs to Li2S4
...
/************
 * Read the xyz in the POSCAR.Li2S4 and POSCAR.MoSe2
************/
void readxyz(int atom)
{
    ...
    for(k = 1; k <= atom; k++)
    {
        ...
        tmp = atoms[k].x;
        atoms[k].x = atoms[k].x * 17.362167 + atoms[k].z * 0.09881305;
        atoms[k].y = atoms[k].y * 16.6879928817;
        atoms[k].z = tmp * 0.1004995 + atoms[k].z * 16.7347398;
    }
    fclose(init);
...
// Calculate the center coordination of Li2S4
...
int main()
{
    int atom = 6;   // No. of atoms of Li2S4
    ...
    readxyz(atom);  // read Li2S4 and MoSe2
    center(atom);   // calculate the center of Li2S4

    rotation(atom, alpha, beta, gamma);  // rotate the Li2S4
